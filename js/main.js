    ...
    try {
      ...
      // unlocked door â€” or pressing Interact near one â€” swaps the room.
      ...
      function saveUnlockedConnections() {
        try {
          ...
          // localStorage unavailable (private mode, etc.) â€” fail silently,
      ...
      function applyDoorLockVisual(record) {
        if (record.locked) {
          record.panelMesh.material.color.set(0x6b3a49); // dim rose â€” locked
          record.panelMesh.material.emissive.set(0x000000);
          record.panelMesh.material.emissiveIntensity = 0;
        } else {
          record.panelMesh.material.color.set(0xffe4a8); // warm glow â€” unlocked
      ...
      function makeMissingPhotoTexture(caption) {
        ...
        if (caption) {
          ...
          const trimmed = caption.length > 28 ? caption.slice(0, 26) + "â€¦" : caption;
      ...
      // the image finished loading â€” so a wide or tall photo could grow
      ...
      // never stall â€” or crash â€” the room.
      ...
      // normal rooms to comfortably reflow through 15â€“30+ exhibits. Room 1
      ...
      // frame from a list of free segments, so later placements â€” automatic
      // or manual â€” never land on top of it.
      ...
      // â€” which is exactly what keeps it off doorways and out of corners â€”
      ...
      function buildPhotoFrameMesh(wallId, offset, photo, frameW, frameH, preloaded, mountHeight = PHOTO_HEIGHT) {
        ...
        // during layout â€” see loadPhotoAspect â€” so this just applies the
      ...
      // decided â€” see loadPhotoAspect above) resolving to the same kind of
      ...
      //      packSegments â€” which guarantees real footprints, minimum
        ...
        items.forEach((item) => {
          ...
          if (pos && typeof pos === "object" && pos.wall) {
            ...
            console.warn(
              ...
              `is invalid (${validation.reason}) â€” using automatic placement instead.`
            );
          }
          autoQueue.push(item);
        });
      ...
      function buildRoomShell(roomId) {
        ...
        // ratio before deciding where anything goes â€” see its comment for
        ...
        if (roomIndicatorEl) {
          roomIndicatorEl.textContent = `Exhibit ${roomCfg.exhibitNo} â€” ${roomCfg.title}`;
        }
      }

      // ---- Milestone 3: door interaction â€” proximity, questions,
      ...
      const WRONG_ANSWER_NUDGES = [
        "Not quite â€” give it another go?",
        "Close, but not it. One more try?",
        "Still not it â€” take a guess, it's okay!",
      ];
      ...
      function submitDoorAnswer() {
        ...
        if (!normalized) {
          doorOverlayFeedbackEl.classList.remove("success");
          doorOverlayFeedbackEl.textContent = "Type an answer first â€” even a guess!";
          return;
        }
        ...
        if (isCorrect) {
          ...
          doorOverlayFeedbackEl.textContent = "Correct â€” the door opensâ€¦";
          doorOverlaySkipEl.classList.add("hidden");
      ...
      function updateInteractPrompt() {
        ...
        if (closestDoor && (!closestPhoto || closestDoorDist <= closestPhotoDist)) {
          ...
          doorPromptEl.textContent = nearDoor.locked
            ? `${nearDoor.label} â€” tap / press E to answer`
            : `${nearDoor.label} â€” walk through`;
      ...
      function spawnPlayerEnteringFrom(targetRoomId, fromRoomId) {
        ...
        if (record) {
          ...
          // whose door layout isn't built yet) â€” safe default spawn.
      ...
      window.addEventListener("keydown", (e) => {
        ...
        // player type/scroll freely â€” don't hijack keystrokes as
      ...
      function updateMovement(delta) {
        ...
        // overlay â€” this is the "walk into a locked door" behaviour.
        if (newX > MOVE_BOUND) {
      ...
      if (statusEl) statusEl.textContent = "Engine â€” Milestone 4: photo frames + lightbox ready";
    } catch (err) {
      // The start screen and Enter button still work even if this fails â€”
      // only the 3D scene underneath is affected.
      console.error("Museum engine failed to initialize:", err);
      if (statusEl) statusEl.textContent = "Engine â€” scene failed to load (see console)";
    }
